...
def patch_termination_manager():
    ...
    NOTE (2025-11, isaac-launchable):
    The IsaacLab version bundled here already includes this fix and also changed the type of `_term_dones`.
    If we re-apply this patch, it breaks with `TypeError: unhashable type: 'slice'`.

    So on this setup, this patch is intentionally a no-op.
    """
    # Do nothing – rely on IsaacLab's own TerminationManager.compute implementation.
    return
...
# Call monkey_patch() so future imports keep the same interface, but nothing is patched.
monkey_patch()
